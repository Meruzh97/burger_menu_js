...
if(menuLinks.length > 0) { //проверка есть ли такие ребята, если есть начинаем работу
    ...
    function onMenuLinkClick(e) {
        ...
        if(menuLink.dataset.goto && document.querySelector(menuLink.dataset.goto)){ //если есть такая тема, закинем ее в константу
            ...
            //^(вверху) мы высчитывеам положение этого объекта обязательно с учетом высоты шапки. Создаем константу и помощью ф-и getBoundingClientRect().top - мы получаем местоположение на странице. (.top - потомучто мы хотим получить расстояние от верха)
